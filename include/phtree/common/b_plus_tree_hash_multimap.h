...
#ifndef PHTREE_COMMON_B_PLUS_TREE_HASH_MULTIMAP_H
#define PHTREE_COMMON_B_PLUS_TREE_HASH_MULTIMAP_H
 ...
 */
    ...
    class b_plus_tree_hash_multiset {
        class bpt_node_base;
        ...
        using TreeT = b_plus_tree_hash_multiset<T, HashT, PredT>;

    public:
        explicit b_plus_tree_hash_multiset() : root_{new NLeafT(nullptr, nullptr, nullptr)}, size_{0} {};

        b_plus_tree_hash_multiset(const b_plus_tree_hash_multiset &other) : size_{other.size_} {
            root_ = other.root_->is_leaf() ? new NLeafT(*other.root_->as_leaf())
                                           : new NInnerT(*other.root_->as_inner());
        }

        b_plus_tree_hash_multiset(b_plus_tree_hash_multiset &&other) noexcept
                : root_{other.root_}, size_{other.size_} {
            other.root_ = nullptr;
            other.size_ = 0;
        }

        b_plus_tree_hash_multiset &operator=(const b_plus_tree_hash_multiset &other) {
            assert(this != &other);
            delete root_;
            root_ = other.root_->is_leaf() ? new NLeafT(*other.root_->as_leaf())
                                           : new NInnerT(*other.root_->as_inner());
        ...
        b_plus_tree_hash_multiset &operator=(b_plus_tree_hash_multiset &&other) noexcept {
            delete root_;
            root_ = other.root_;
            other.root_ = nullptr;
            size_ = other.size_;
        ...
        ~b_plus_tree_hash_multiset() {
            delete root_;
            root_ = nullptr;
        }

#ifdef BPTHMM
        [[nodiscard]] auto find(const hash_t &hash) {
            auto node = root_;
            while (!node->is_leaf()) {
            ...
            return node->as_leaf()->find(hash);
        }

        [[nodiscard]] auto find(const hash_t &hash) const {
            return const_cast<b_plus_tree_hash_multiset &>(*this).find(hash);
        }
        ...
        [[nodiscard]] auto find(const T &value) const {
            return const_cast<b_plus_tree_hash_multiset &>(*this).find(value);
        }
        ...
        [[nodiscard]] size_t count(const T &value) const {
            return const_cast<b_plus_tree_hash_multiset &>(*this).find(value) != end();
        }
...
//            auto hash = (hash_t) HashT{}(value);
//            while (!node->is_leaf()) {
//                // TODO remove as_inner()
//                node = node->as_inner()->lower_bound(hash)->second;
//                if (node == nullptr) {
//                    return end();
//                }
//            }
//            return IterT(node, node->as_leaf()->lower_bound(hash));
//        }
//
//        [[nodiscard]] auto lower_bound(const T& value) const {
//            return const_cast<b_plus_tree_hash_multiset &>(*this).lower_bound(value);
//        }

#ifdef BPTHMM
        [[nodiscard]] auto lower_bound(const hash_t& hash) {
            auto node = root_;
            while (!node->is_leaf()) {
                // TODO remove as_inner()
                auto iter = node->as_inner()->lower_bound(hash);
                if (iter == node->as_inner()->data_.end()) {
                    return end();
                }
                node = iter->second;
            }
            return IterT(node->as_leaf(), node->as_leaf()->lower_bound(hash));
        }

        [[nodiscard]] auto lower_bound(const hash_t& hash) const {
            return const_cast<b_plus_tree_hash_multiset &>(*this).lower_bound(hash);
        }
#endif
        ...
        auto emplace_hint(const IterT &hint, Args &&... args) {
            ...
            return node->try_emplace(hash, *this, size_, std::move(t));
#else
            // The following may drop a valid hint but is easy to check.
            if (node->data_.begin()->first > hash || (node->data_.end() - 1)->first < hash) {
        ...
        auto erase(const IterT &begin, const IterT &end) {
            ...
            auto result = erase_entries(begin, end, *this);
            if (result.node_) {
                return IterT(static_cast<NLeafT *>(result.node_), result.iter_);
            }
            return IterT();
        ...
        auto erase_entries(IterT it_begin, IterT it_end, TreeT &tree) {
//            _check();
//            auto guard = [this]() {
//                this->_check();
//            };
            NLeafT * current = it_begin.node_;
            auto current_begin = it_begin.iter_;
            while (current != it_end.node_ && current->next_node_ != nullptr) {
                auto old_size = current->data_.size();
                hash_t max_key_old = current->data_.back().first;
                current->data_.erase(current_begin, current->data_.end());
                size_ -= (old_size - current->data_.size());
                current->check_merge(true,  current->data_.end(), max_key_old, tree);
                current = current->next_node_;
                assert(current != nullptr);
                current_begin = current->data_.begin();
            }
            auto old_size = current->data_.size();
            hash_t max_key_old = current->data_.back().first;
            auto next_entry = current->data_.erase(current_begin, it_end.is_end() ? current->data_.end() : it_end.iter_);
            size_ -= (old_size - current->data_.size());
            // TODO remove TER argument...? Or iterator argument?
            bool ter = it_end.is_end() || it_end.iter_ == it_end.node_->data_.end();
            auto result = current->check_merge(ter,  next_entry, max_key_old, tree);
            if (result.node_) {
    ...
    private:
        ...
        public:
            ...
            [[nodiscard]] inline size_t M_min() const noexcept {
                return this->is_leaf_ ? M_leaf_min : M_inner_min;
            }

            [[nodiscard]] inline size_t M_max() const noexcept {
                return this->is_leaf_ ? M_leaf : M_inner;
            }

            [[nodiscard]] inline size_t M_init() const noexcept {
            ...
            // TODO merge with erase_entry()
            auto check_merge(bool tail_entry_erased, DataIteratorT iter_after_erased, hash_t max_key_old, TreeT &tree) {
                using ER = EraseResult;
                auto &parent_ = this->parent_;
                //hash_t max_key_old = data_.back().first;

                auto result = iter_after_erased;
//                bool tail_entry_erased = result == data_.end();
 //               if (parent_ == nullptr) {
                // For erase begin-end we need to avoid using data_.begin(). However, this only happens for LeaFNodes.
                if (parent_ == nullptr) {
                    if constexpr (std::is_same_v<ThisT, NInnerT>) {
                        if (data_.size() < 2) {
                            auto remaining_node = data_.begin()->second;
        ...
        public:
            ...
            [[nodiscard]] IterT find(hash_t hash) noexcept {
                ...
                if (!iter_full.is_end() && iter_full.hash() == hash) {
                    return iter_full;
                }
                return IterT();
            }
            ...
            auto try_emplace(hash_t hash, TreeT &tree, size_t &entry_count, T &&t) {
                ...
                if (it != this->data_.end() && it->first == hash) {
                    ...
                    while (!full_iter.is_end() && full_iter.hash() == hash) {
                        // TODO Remove this loop from BPTHMM?!?!?
#ifndef BPTHMM
        ...
        class bpt_iterator {
            using EntryT = typename b_plus_tree_hash_multiset<T, HashT, PredT>::bpt_entry_leaf;
            friend b_plus_tree_hash_multiset<T, HashT, PredT>;
        ...
        public:
            ...
            explicit bpt_iterator(NLeafT *node, LeafIteratorT it) noexcept
                    : node_{it == node->data_.end() ? nullptr : node}, iter_{node_ == nullptr ? LeafIteratorT{} : it} {
    ...
    class b_plus_tree_hash_multimap {
        class iterator;
    ...
    public:
        b_plus_tree_hash_multimap() : map_{} {};

        b_plus_tree_hash_multimap(const b_plus_tree_hash_multimap &) = default;

        b_plus_tree_hash_multimap(b_plus_tree_hash_multimap &&) noexcept = default;

        b_plus_tree_hash_multimap &operator=(const b_plus_tree_hash_multimap &) = default;

        b_plus_tree_hash_multimap &operator=(b_plus_tree_hash_multimap &&) noexcept = default;

        ~b_plus_tree_hash_multimap() = default;
...
#ifdef BPTHMM
            return IterT(map_.find(HashT{}(key)));
#else
...
#ifdef BPTHMM
        // TODO Test performance with direct map implementation (or friend impls on Set), i.e. avoid EntryT construction
        [[nodiscard]] auto lower_bound(const KeyT &key) const {
            // TODO see non-const Find() in Node (line 244)
//            const EntryT e{key, ValueT{}};
//            //const EntryT e = std::make_pair<KeyT, ValueT>(key, {});
//            return IterT(map_.lower_bound(e));//EntryT{key, {}}));
            return IterT(map_.lower_bound(key));//EntryT{key, {}}));
        }
#endif
...
#ifdef BPTHMM
            return iterator(map_.emplace(key, std::forward<Args>(args)...));
#else
        ...
        auto erase(const IterT &begin, const IterT &end) {
            return IterT(map_.erase(begin.map_iter_, end.map_iter_));
        }
    ...
    private:
        ...
        class iterator {
            using T = EntryT;
            using MapIterType =
                    decltype(std::declval<b_plus_tree_hash_multiset<EntryT, EntryHashT, EntryEqualsT>>()
                            .begin());
            friend b_plus_tree_hash_multimap<KeyT, ValueT, HashT, PredT>;
        ...
        b_plus_tree_hash_multiset<EntryT, EntryHashT, EntryEqualsT> map_;
    };

}  // namespace improbable::phtree

#endif  // PHTREE_COMMON_B_PLUS_TREE_HASH_MULTIMAP_H
