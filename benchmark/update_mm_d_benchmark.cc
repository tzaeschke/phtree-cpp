...
namespace {
    ...
    enum Scenario {
        ERASE_EMPLACE, MM_BPT_RELOCATE, MM_SET_RELOCATE, MM_SET_RELOCATE_IF
    };
    ...
    //           PhTreeMultiMapD<DIM, payload_t, CONVERTER<SCENARIO, DIM>, absl::flat_hash_set<payload_t>>>>;
//            PhTreeMultiMapD<DIM, payload_t, CONVERTER<SCENARIO, DIM>, robin_hood::unordered_set<payload_t>>>>;
    ...
    IndexBenchmark<DIM, SCENARIO>::IndexBenchmark(
            ...
            : data_type_{static_cast<TestGenerator>(state.range(1))}, num_entities_(state.range(0)),
              updates_per_round_(updates_per_round), move_distance_(std::move(move_distance)), points_(num_entities_),
              updates_(updates_per_round), random_engine_{0},
...
BENCHMARK_CAPTURE(PhTreeMMRelocateIfStdSet3D, UPDATE_1000, UPDATES_PER_ROUND)
        ...
        ->Ranges({{1000,                1000 * 1000},
                  {TestGenerator::CUBE, TestGenerator::CLUSTER}})
