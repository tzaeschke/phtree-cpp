...
namespace phtree_multimap2_d_test {
...
using TestTree = PhTreeMultiMap2D<DIM, T>;
...
namespace std {
template <>
struct hash<phtree_multimap2_d_test::Id> {
    size_t operator()(const phtree_multimap2_d_test::Id& x) const {
...
template <dimension_t DIM>
void generateCube(std::vector<TestPoint<DIM>>& points, size_t N, size_t dupl_per_coord = NUM_DUPL) {
    assert(N % dupl_per_coord == 0);
...
TEST(PhTreeMMDTest, TestEmplace) {
    ...
    for (int i = 0; i < (int)N; i++) {
        TestPoint<dim>& p = points.at(i);
        Id id(i);
        auto iter = tree.emplace(p, id);
        ASSERT_EQ(true, iter.second);
        ASSERT_EQ(tree.count(p), i % NUM_DUPL + 1);
        ASSERT_EQ(id._i, tree.find(p, id)->_i);
        ASSERT_EQ(i + 1, tree.size());
...
void populate(
    ...
    size_t N,
    bool shuffle = false,
    size_t dupl_per_coord = NUM_DUPL) {
    generateCube(points, N, dupl_per_coord);
    if (shuffle) {
...
void TestUpdateWithRelocate(bool relocate_to_existing_coordinate, bool shuffle, size_t duplicates) {
    const dimension_t dim = 3;
    TestTree<dim, Id> tree;
    size_t N = 1000;
    std::array<double, 4> deltas{0, 0.1, 1, 10};
    std::vector<TestPoint<dim>> points;
    populate(tree, points, N, shuffle, duplicates);
    std::vector<std::pair<TestPoint<dim>, size_t>> entries;
    for (size_t i = 0; i < N; ++i) {
        entries.emplace_back(points[i], i);
    }
        ...
        for (auto delta : deltas) {
            ...
            for (auto& e : entries) {
                ...
                if (relocate_to_existing_coordinate) {
                    pNew = delta > 0.0 ? entries[(i + 17) % N].first : pOld;
                } else {
...
TEST(PhTreeMMDTest, TestUpdateWithRelocateDelta) {
    TestUpdateWithRelocate(false, false, NUM_DUPL);
}

TEST(PhTreeMMDTest, TestUpdateWithRelocateToExisting) {
    TestUpdateWithRelocate(true, false, NUM_DUPL);
}

TEST(PhTreeMMDTest, TestUpdateWithRelocateDeltaShuffled) {
    TestUpdateWithRelocate(false, true, NUM_DUPL);
}

TEST(PhTreeMMDTest, TestUpdateWithRelocateToExistingShuffled) {
    TestUpdateWithRelocate(true, true, NUM_DUPL);
}

TEST(PhTreeMMDTest, TestUpdateWithRelocateDelta100) {
    TestUpdateWithRelocate(false, false, 100);
}

TEST(PhTreeMMDTest, TestUpdateWithRelocateToExisting100) {
    TestUpdateWithRelocate(true, false, 100);
}

TEST(PhTreeMMDTest, TestUpdateWithRelocateDeltaShuffled100) {
    TestUpdateWithRelocate(false, true, 100);
}

TEST(PhTreeMMDTest, TestUpdateWithRelocateToExistingShuffled100) {
    TestUpdateWithRelocate(true, true, 100);
}
...
TEST(PhTreeMMDTest, TestUpdateWithRelocateCornerCases) {
    ...
    // check that existing destination does not fail
    tree.emplace(point0, Id(1));
...
TEST(PhTreeMultimap2Test, FuzzTest1) {
    const dimension_t DIM = 1;
    using Value = std::uint8_t;
    PhTreeMultiMap2<DIM, Value, ConverterNoOp<DIM, std::int64_t>> tree{};
    tree.emplace({0}, 103);
    tree.emplace({0}, 0);
    tree.emplace({0}, 0);
    tree.emplace({0}, 0);
    ...
    tree.emplace({126}, 94);
    tree.emplace({94}, 255);
    tree.emplace({255}, 255);
    tree.relocate({94}, {136}, 255);
    tree.emplace({0}, 255);
    tree.relocate({255}, {176}, 255);
}
