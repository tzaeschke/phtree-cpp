...
void SmokeTestMap() {
    ...
    size_t val = 0;
    for (int i = 0; i < 10; i++) {
        b_plus_tree_hash_multimap<Id, size_t, HashT, std::equal_to<>> test_map;
        std::unordered_multimap<Id, size_t> reference_map;
        std::map<size_t, Id> reverse_map;
        for (int j = 0; j < N; j++) {
            ...
            if (key % 6 == 0) {
                CheckMapResult(test_map.emplace(id, val), test_map.end(), id, val);
            } else if (key % 6 == 1) {
                CheckMapResult(test_map.try_emplace(id, val), test_map.end(), id, val);
            } else if (key % 6 == 2) {
            ...
            for (auto& entry : test_map) {
                Id& k = entry.first;
                auto& eMap = *test_map.find(k);
                ASSERT_EQ(entry.first, eMap.first);
                ASSERT_LE(prev_key._i, eMap.first._i);
                prev_key = eMap.first;
                auto& eRef = *reverse_map.find(eMap.second);
...
TEST(PhTreeBptHashMultiapTest, SmokeTestNonUnique) {
    SmokeTestMap<std::hash<Id>>();
}

//TEST(PhTreeBptHashMultiapTest, SmokeTestSameHash) {
//    struct DumbHash {
...
void SmokeTestSet() {
    ...
    for (int i = 0; i < 10; i++) {
        b_plus_tree_hash_multiset<Id, Hash> test_map;
        std::unordered_multiset<Id> reference_map;
        for (int j = 0; j < N; j++) {
            {
                size_t key = cube_distribution(random_engine);
                Id id(key);
                ...
                if (key % 3 == 0) {
                    if (key % 2 == 0) {
                        CheckSetResult(test_map.emplace(id), test_map.end(), id);
                    } else {
                        CheckSetResult(test_map.emplace(key), test_map.end(), id);
                    }
                } else if (key % 3 == 1) {
                    // Leaf-hint of questionable quality
                    auto hint = test_map.find(Id(key - 1));
...
TEST(PhTreeBptHashMultiapTest, SmokeTestWithTryEmplace) {
    const int N = 200;
    std::default_random_engine random_engine{0};
    std::uniform_int_distribution<> cube_distribution(0, N / 2);

    for (int i = 0; i < 10; i++) {
        b_plus_tree_hash_multimap<size_t, size_t, std::hash<size_t>, std::equal_to<>> test_map;
...
void SmokeTestWithErase(bool by_iterator) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (auto key : key_list) {
            // TODO We have always val==key, this test may overlook some complications....
            Id id(key);
            auto val = key;
            ...
            if (by_iterator) {
                while (it != test_map.end() && it->second != val) {
                    ++it;
                }
                auto next = it;
                ++next;
                ...
                auto ref_iter = reference_map.find(id);
                ...
                // TODO
//                if (is_last) {
//                    ASSERT_EQ(reference_map.end(), reference_map.erase(ref_iter));
//                } else {
//                    ASSERT_NE(reference_map.end(), reference_map.erase(ref_iter));
//                }
            } else {
               // TODO return erase-count
                test_map.erase(id);
...
TEST(PhTreeBptHashMultiapTest, SmokeTestWithErase) {
    SmokeTestWithErase<std::hash<Id>>(true);
    // TODO
//    SmokeTestWithErase<std::hash<Id>>(false);
}

TEST(PhTreeBptHashMultiapTest, SmokeTestWithEraseSameHash) {
    struct DumbHash {
        size_t operator()(const Id&) const {
            return 42;
        }
