...
#include "phtree/common/bpt_vector_tree.h"
...
void SmokeTest() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        for (size_t j = 0; j < N; j++) {
            ...
            test_map[pos].i_ += 17;
            reference_map[pos].i_ += 17;
            ASSERT_EQ(test_map[pos], reference_map[pos]);
            ASSERT_EQ(test_map.front(), reference_map.front());
        ...
        for (size_t j = 0; j < N; j++) {
            ...
            ASSERT_EQ(test_map[pos], reference_map[pos]);
            test_map.erase_back();
            reference_map.erase(reference_map.end() - 1);
        }
...
TEST(PhTreeBptVectorTreeTest, SmokeTest) {
...
TEST(PhTreeBptVectorTreeTest, SmokeTest_TriviallyCopyable) {
...
TEST(PhTreeBptVectorTreeTest, TestCopyConstruct) {
    using TestTree = vector_tree<Id>;
    TestTree tree1;
...
TEST(PhTreeBptVectorTreeTest, TestCopyAssign) {
    using TestTree = vector_tree<Id>;
    TestTree tree1;
    tree1.emplace_back(Id(1));
...
TEST(PhTreeBptVectorTreeTest, TestMoveConstruct) {
    using TestTree = vector_tree<Id>;
    TestTree tree1;
    tree1.emplace_back(Id(1));
...
TEST(PhTreeBptVectorTreeTest, TestMoveAssign) {
    using TestTree = vector_tree<Id>;
    TestTree tree1;
    tree1.emplace_back(Id(1));
