...
namespace phtree_multimap2_box_d_test {
...
using TestTree = PhTreeMultiMap2BoxD<DIM, T>;
...
namespace std {
template <>
struct hash<phtree_multimap2_box_d_test::Id> {
    size_t operator()(const phtree_multimap2_box_d_test::Id& x) const {
...
TEST(PhTreeMMBoxDTest, SmokeTestTreeAPI) {
    ...
    PhTreeMultiMap2BoxD<3, Id*> treePtr;
    Id* idPtr = new Id(1);
