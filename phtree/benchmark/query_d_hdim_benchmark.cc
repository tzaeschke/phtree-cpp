...
template <typename... Arguments>
void PhTree10D_MMFE(benchmark::State& state, Arguments&&... arguments) {
    IndexBenchmark<10, MIN_MAX_FOR_EACH> benchmark{state, arguments...};
    benchmark.Benchmark(state);
}

template <typename... Arguments>
void PhTree15D_MMFE(benchmark::State& state, Arguments&&... arguments) {
    IndexBenchmark<15, MIN_MAX_FOR_EACH> benchmark{state, arguments...};
    benchmark.Benchmark(state);
}

template <typename... Arguments>
void PhTree20D_MMFE(benchmark::State& state, Arguments&&... arguments) {
    IndexBenchmark<20, MIN_MAX_FOR_EACH> benchmark{state, arguments...};
    benchmark.Benchmark(state);
}
...
// PhTree 3D CUBE
BENCHMARK_CAPTURE(PhTree3D_MMFE, WQ_CU_3D_1M, TestGenerator::CUBE, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree6D_MMFE, WQ_CU_6D_1M, TestGenerator::CUBE, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree10D_MMFE, WQ_CU_10D_1M, TestGenerator::CUBE, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree15D_MMFE, WQ_CU_15D_1M, TestGenerator::CUBE, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree20D_MMFE, WQ_CU_20D_1M, TestGenerator::CUBE, SIZE)
    ->Unit(benchmark::kMillisecond);

// index type, scenario name, data_type, num_entities, query_result_size
// PhTree 3D CLUSTER
BENCHMARK_CAPTURE(PhTree3D_MMFE, WQ_CL_3D_1M, TestGenerator::CLUSTER, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree3D_MMFE, WQ_CL_6D_1M, TestGenerator::CLUSTER, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree3D_MMFE, WQ_CL_10D_1M, TestGenerator::CLUSTER, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree3D_MMFE, WQ_CL_15D_1M, TestGenerator::CLUSTER, SIZE)
    ->Unit(benchmark::kMillisecond);

BENCHMARK_CAPTURE(PhTree3D_MMFE, WQ_CL_20D_1M, TestGenerator::CLUSTER, SIZE)
    ->Unit(benchmark::kMillisecond);
